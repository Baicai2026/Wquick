...
static std::string findGxx() {
    ...
    // 3. wqc.exe 同目录下的 TDM-GCC-64/bin/g++.exe（内置 gcc）
    std::string builtin = exeDir() + "TDM-GCC-64\\bin\\g++.exe";
    if (GetFileAttributesA(builtin.c_str()) != INVALID_FILE_ATTRIBUTES) return builtin;
    // 4. PATH（返回裸命令，让 cmd 自己搜）
    return "g++";
}
...
int main(int argc, char* argv[]) {
    ...
    std::string compileArgs =
        "\"" + gccPath + "\" -std=c++17 -O2 -o \"" + binFile + "\" \"" + tmpCpp + "\" -lpthread -pthread";
